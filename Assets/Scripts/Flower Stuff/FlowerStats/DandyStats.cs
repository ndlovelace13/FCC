...
public class DandyStats : FlowerStats
{
    ...
    void Start()
    {
        description = "Prior to the replicant threat, the Dandelion was a controversial weed, hated by many for its seemingly endless ability to reproduce. Those haters are noticeably silent following the discovery of dandelion's remarkable extermination power.";
    ...
    IEnumerator ProjSpawn(float angle, GameObject original)
    {
        ...
        proj.GetComponent<ProjectileBehavior>().SetProps(range / 2f, damage / splitCount, actualAugs, projDir, original.GetComponent<ProjectileBehavior>().single, false);
